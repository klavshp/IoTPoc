...
namespace ProcessDeviceToCloudMessages
{
    ...
    {
        private const int MaxBlockSize = 4 * 1024;               //  4 * 1024 * 1024;
        ...
        public StoreEventProcessor()
        {
            ...
            _blobContainer = blobClient.GetContainerReference(Config.Config.BlobContainer);
        ...
        async Task IEventProcessor.ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
        {
            foreach (EventData eventData in messages)
            {
                ...
                if (eventData.Properties.ContainsKey("messageType") &&
                    (string) eventData.Properties["messageType"] == "interactive")
                {
                    var messageId = (string) eventData.SystemProperties["message-id"];

                    var queueMessage = new BrokeredMessage(new MemoryStream(data)) {MessageId = messageId};
                ...
                Console.WriteLine($"Message received.  Partition: '{context.Lease.PartitionId}', Data: '{Encoding.UTF8.GetString(data)}'");
            }
        }
        ...
        private async Task AppendAndCheckpoint(PartitionContext context)
        {
            ...
            {
                await currentBlob.PutBlockAsync(blockId, _toAppend, Convert.ToBase64String(md5));
                var newBlockList = new List<string> {blockId};
                await currentBlob.PutBlockListAsync(newBlockList);

                WriteHighlightedMessage($"Created new blob: {currentBlob.Name}");
            }
